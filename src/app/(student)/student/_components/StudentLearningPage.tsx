...
export default function StudentLearningPage({
  ...
  const startSession = () => {
    if (isExam && !workspace?.activeExam) {
  ...
  const submitAnswer = async () => {
    ...
    if (nextIndex >= problemCount) {
      ...
      setShowReviewList(!isExam && nextWrongCount > 0);
      setResultMessage(
          ...
          {!isRunning ? (
            <>
              <S.LearningHero>
                ...
                <S.LearningHeroText>
                  <span>{isExam ? "시험 탭" : "연습 탭"}</span>
                  <strong>{isExam ? "시험을 준비해요" : "구구단 연습을 시작해요"}</strong>
                  <p>
                    {isExam
                      ? "선생님이 정한 시간에만 시험을 시작할 수 있어요."
                      : "틀린 문제는 오답노트에서 다시 볼 수 있어요."}
                  </p>
                </S.LearningHeroText>
                <S.GoalRing>
              ...
              <S.ColorSection>
                <S.ColorSectionTitle>{isExam ? "시험 문제 종류" : "연습 종류"}</S.ColorSectionTitle>
                <S.ModeCardGrid>
                  <S.ModeCardButton
                    type="button"
              ...
              <S.PrimaryPlayActions $columns={isExam ? 1 : 2}>
                <S.StartButton
                  type="button"
                  $tone={isExam ? "orange" : "purple"}
                  onClick={startSession}
                  disabled={isExam && !workspace.activeExam}
                >
                  {isExam ? "시험 시작" : "연습 시작"}
                </S.StartButton>
                {!isExam ? (
              ...
              {isExam && !workspace.activeExam ? (
                <S.PlayHint>현재 시험 응시 시간이 아닙니다.</S.PlayHint>
              ) : null}
            </>
          ) : (
            <S.StudyStage>
